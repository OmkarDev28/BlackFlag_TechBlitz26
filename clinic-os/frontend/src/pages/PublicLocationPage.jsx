    ...
    <PublicLayout>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Clinic address, hours & directions</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20, alignItems: 'start' }}>
        ...
        <div className="card" style={{ overflow: 'hidden', padding: 0 }}>
          ...
          <div style={{ padding: '14px 20px', display: 'flex', gap: 10, borderTop: '1px solid var(--border)' }}>
            <a href={googleMapsLink} target="_blank" rel="noreferrer" className="btn btn-outline btn-sm">🗺 Open in Google Maps</a>
            <a href={directionsLink} target="_blank" rel="noreferrer" className="btn btn-outline btn-sm">🧭 Directions</a>
          </div>
        </div>
            ...
            <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div>
                ...
                <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: 1.7 }}>
                  {clinic.address}<br />{clinic.city}, {clinic.state}<br />PIN: {clinic.pincode}
                </div>
              </div>
          ...
          <div className="card">
            ...
            <div className="card-body">
              {clinic.hours.map(({ day, time }) => {
                ...
                const isToday = (day.includes('Monday') && d >= 1 && d <= 5) ||
                  (day === 'Saturday' && d === 6) || (day === 'Sunday' && d === 0);
                return (
                  <div key={day} style={{
                    ...
                    <span style={{ fontSize: '0.85rem', color: time === 'Closed' ? 'var(--red)' : isToday ? 'var(--accent-dark)' : 'var(--text-muted)', fontWeight: isToday ? 600 : 400 }}>
                      {time}
                    </span>
                  </div>
                );
          ...
          <div className="card">
            ...
            <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: '0.88rem', color: 'var(--text-muted)' }}>
              {[
                ...
                ['🛺', 'Auto/Cab', 'Drop-off point at main gate'],
              ].map(([icon, mode, detail]) => (
                <div key={mode} style={{ display: 'flex', gap: 10 }}>
                  <span>{icon}</span>
                  <div><span style={{ fontWeight: 500, color: 'var(--text)' }}>{mode}: </span>{detail}</div>
                </div>
              ))}
            </div>
          </div>
